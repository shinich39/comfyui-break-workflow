...
const CLASS_NAME = "BreakWorkflow";
...
async function handleFile(file) {
  ...
  if (Settings["Debug"]) {
    console.log("[comfyui-break-workflow] handleFile() Workflow\n", workflow);
    console.log("[comfyui-break-workflow] handleFile() Prompt\n", prompt);
  }
  ...
  if (workflow && prompt) {
    ...
    for (let i = 0; i < batchCount; i++) {
      try {
        ...
        // if (Settings["Debug"]) {
        //   console.log("[comfyui-break-workflow]\n", res);
        // }
      } catch (err) {
        console.error(err);
      }
...
app.registerExtension({
	name: "shinich39.BreakWorkflow",
  settings: [
    {
      id: 'shinich39.BreakWorkflow.Debug',
      category: ['BreakWorkflow', 'Pray for modularism', 'Debug'],
      name: 'Debug',
    ...
    {
      id: 'shinich39.BreakWorkflow.RemoveBreakAfterGeneration',
      category: ['BreakWorkflow', 'Pray for modularism', 'RemoveBreakAfterGeneration'],
  ...
  setup() {
    ...
    setTimeout(() => {
      ...
      api.queuePrompt = async function(...args) {
        ...
        if (Settings["Debug"]) {
          console.log(`[comfyui-break-workflow] api.queuePrompt() workflow.${PROP_NAME}`, workflow[PROP_NAME]);
        }
        ...
        if (Settings["Debug"]) {
          console.log("[comfyui-break-workflow] api.queuePrompt() Output\n", output);
          console.log(`[comfyui-break-workflow] api.queuePrompt() Workflow\n`, workflow);
        }

        return await origQueuePrompt.apply(this, arguments);
      }
  
      console.log("[comfyui-break-workflow] initialized");
    }, 1024 * 1);
  },
  nodeCreated(node) {
    if (node.comfyClass === CLASS_NAME) {
      ...
      const b = node.addWidget("button", "Drop images here to continue", null, () => {}, { serialize: false, });
      b.computeSize = () => [0, 26];
      b.callback = async () => {
